...
public:
        ...
        const CarEngine & GetEngine() const {
                return engine;
        }
        const CarClutch & GetClutch() const {
                return clutch;
        }
        const CarTransmission & GetTransmission() const {
                return transmission;
        }
        const CarBrake & GetBrake(WheelPosition pos) const {
                return brake[pos];
        }
        const CarWheel & GetWheel(WheelPosition pos) const {
                return wheel[pos];
        }
        const CarTire & GetTire(WheelPosition pos) const {
                return tire[pos];
        }
        btScalar GetFuelAmount() const {
                return fuel_tank.FuelPercent();
        }
        ...
        const CarSuspension & GetSuspension(WheelPosition pos) const {
                return suspension[pos];
        }
        ...
        DynamicsWorld * getDynamicsWorld() const {
                return world;
        }
